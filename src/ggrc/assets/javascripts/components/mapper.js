...
(function (can, $) {
  ...
  var MapperModel = can.Map({
    ...
    get_instance: can.compute(function () {
      return CMS.Models.get_instance(
        this.attr('object'), this.attr('join_object_id'));
    }),
    ...
    types: can.compute(function () {
      ...
      selectorList = _.intersection.apply(_, _.compact([_.keys(canonical), list]));
      selectorList = _.union(selectorList, whitelist);
      can.each(selectorList, function (model_name) {
        if (!model_name || !CMS.Models[model_name] || ~forbidden.indexOf(model_name)) {
          return;
        }
        cmsModel = CMS.Models[model_name];
  ...
  can.Component.extend({
    ...
    scope: function (attrs, parentScope, el) {
      ...
      var tree_view = GGRC.tree_view.sub_tree_for[object];

      if ($el.attr('search-only')) {
        data.search_only = /true/i.test($el.attr('search-only'));
      }
      if (parentScope.attr('generate-assessments')) {
        data.generate_assessments = /true/i.test(
          parentScope.attr('generate-assessments'));
      }
    ...
    events: {
      ...
      deferredSave: function () {
        ...
        this.scope.attr('deferred_to').controller.element.trigger('defer:add',
          [data, {map_and_save: true}]);
      ...
      addNew: function (el, ev, model) {
        ...
        binding = selected.get_binding(get_binding_name(
          selected, model.constructor.table_plural));
      ...
      '.modal-footer .btn-map click': function (el, ev) {
        ...
        // handle assessments generation, if callback was specified
      ...
      setModel: function () {
        ...
        this.scope.attr('mapper.model',
          this.scope.mapper.model_from_type(type));
      },
      ...
      allSelected: function () {
        ...
        this.scope.attr('mapper.all_selected',
    ...
    helpers: {
      get_title: function (options) {
        ...
        return instance && instance.title ? instance.title :
  ...
  can.Component.extend({
    ...
    scope: {
      ...
      checkbox: can.compute(function (status) {
        return /true/gi.test(this.attr('is_mapped')) ||
    ...
    events: {
      '{scope} selected': function () {
        this.element.find('.object-check-single').prop('checked',
      ...
      '.object-check-single change': function (el, ev) {
        ...
        scope = this.scope;
        uid = Number(scope.attr('instance_id'));
  ...
  can.Component.extend({
    ...
    events: {
      ...
      '.object-check-all click': function (el, ev) {
        ...
        que.enqueue(_.pluck(entries, 'instance')).trigger()
      ...
      drawPage: function () {
        ...
        next_page = page + 1;
        per_page = Number(this.scope.attr('items-per-page'));
        page_items = this.scope.attr('entries').slice(
          page * per_page, next_page * per_page);
      ...
      searchFor: function (data) {
        ...
        join_model = GGRC.Mappings.join_model_name_for(
        ...
        return GGRC.Models.Search.search_for_types(data.term || '',
          data.model_name, data.options);
      },
      getProgramControls: function () {
        var audit = GGRC.page_instance();
        var controls = audit.get_binding('program_controls');
      ...
      getResults: function () {
        ...
        model_name = this.scope.attr('type');
        contact = this.scope.attr('contact');
        relevantList = this.scope.attr('mapper.relevant');
        ...
        filters = _.compact(_.map(relevantList, function (relevant) {
          var mappings;
          var Loader;
          var relevantMappings;
          if (!relevant.value) {
            return undefined;
          }
          if (model_name === 'AllObject') {
            Loader = GGRC.ListLoaders.MultiListLoader;
            relevantMappings = GGRC.Mappings.get_mappings_for(
              relevant.filter.constructor.shortName);
            mappings = _.compact(_.map(relevantMappings, function (mapping) {
            ...
            mappings = GGRC.Mappings.get_canonical_mapping_name(
              relevant.model_name, model_name);

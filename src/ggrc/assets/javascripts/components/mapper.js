  ...
  can.Component.extend({
    ...
    events: {
      ...
      '.modal-footer .btn-map click': function (el, ev) {
        ...
        var instance = CMS.Models[object].findInCacheById(this.scope.attr('mapper.join_object_id'));
        var mapping;
        var Model;
        var data = {};
        var defer = [];
        var que = new RefreshQueue();
        var assessmentsCallback = this.scope.attr('mapper.assessments_callback');
      ...
      'setBinding': function () {
        ...
        var table_plural = get_binding_name(selected, this.scope.attr('mapper.model.table_plural'));
        var binding;
  ...
  can.Component.extend({
    ...
    events: {
      ...
      '.object-check-single change': function (el, ev) {
        ...
        var uid = +scope.attr('instance_id');
  ...
  can.Component.extend({
    ...
    events: {
      ...
      '{scope.entries} add': function (list, ev, added) {
        ...
        var option = this.getItem(instance);
      ...
      '.object-check-all click': function (el, ev) {
        ...
        var que = new RefreshQueue();
        var entries = this.scope.attr('entries');
      ...
      'getItem': function (model) {
        ...
        var selected = this.scope.attr('mapper.get_instance');
        var mapper = this.scope.mapper.model_from_type(model.type);
        var binding;
        var bindings = this.scope.attr('mapper.bindings');
      ...
      'drawPage': function () {
        ...
        var per_page = +this.scope.attr('items-per-page');
        var page_items = this.scope.attr('entries').slice(page * per_page, next_page * per_page);
